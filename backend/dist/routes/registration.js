...
router.post("/request", async (req, res) => {
    try {
        ...
        const existingRequest = await db.get("SELECT id FROM registration_requests WHERE email = ? AND status = 'pending'", [email]);
        if (existingRequest) {
...
router.get("/requests", async (req, res) => {
    try {
        ...
        const requests = await db.run(`SELECT * FROM registration_requests 
...
router.post("/approve/:requestId", async (req, res) => {
    try {
        ...
        // Obtener solicitud
        const request = await db.get("SELECT email, name, student_id FROM registration_requests WHERE id = ? AND status = 'pending'", [requestId]);
        if (!request) {
    ...
    `, [request.email, passwordHash, request.name, request.student_id]);
        ...
        await db.exec("UPDATE registration_requests SET status = 'approved', processed_at = datetime('now') WHERE id = ?", [requestId]);
        res.json({
            success: true,
...
router.post("/reject/:requestId", async (req, res) => {
    try {
        ...
        await db.exec("UPDATE registration_requests SET status = 'rejected', reason = ?, processed_at = datetime('now') WHERE id = ?", [reason || "No especificado", requestId]);
        res.json({
            success: true,
 ...
 */
router.get("/elections-by-email/:email", async (req, res) => {
    try {
       ...
       ORDER BY e.start_time ASC`, [domain]);
        res.json({
            domain,
            elections: elections || [],
