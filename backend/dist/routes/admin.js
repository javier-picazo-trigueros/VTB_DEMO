...
router.patch("/registration-requests/:id", authAdminMiddleware, async (req, res) => {
    try {
        ...
        if (action === 'approve') {
            ...
            // Insertar usuario
            await db.exec(`INSERT INTO users (email, password_hash, name, student_id, role, is_eligible, created_at)
         VALUES (?, ?, ?, ?, 'voter', 1, CURRENT_TIMESTAMP)`, [request.email, passwordHash, request.full_name, request.student_id]);
