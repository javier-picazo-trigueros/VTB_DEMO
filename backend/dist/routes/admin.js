...
const authAdminMiddleware = async (req, res, next) => {
    try {
        ...
        const admin = await db.get("SELECT role, admin_domain FROM users WHERE id = ?", [decoded.userId]);
...
const getAdminDomain = (req) => req.user?.adminDomain || null;
/**
...
router.get("/dashboard", authAdminMiddleware, async (req, res) => {
    try {
        ...
        if (isSuper) {
            ...
            adminCount = await db.get("SELECT COUNT(*) as count FROM users WHERE role IN ('admin','superadmin')");
            studentCount = await db.get("SELECT COUNT(*) as count FROM users WHERE role IN ('student','voter')");
            totalElections = await db.get("SELECT COUNT(*) as count FROM elections");
            nullifierAudit = await db.get("SELECT COUNT(*) as count FROM nullifier_audit");
        }
        else {
            totalUsers = await db.get("SELECT COUNT(*) as count FROM users WHERE email LIKE '%@' || ?", [adminDomain]);
            adminCount = await db.get("SELECT COUNT(*) as count FROM users WHERE role = 'admin' AND admin_domain = ?", [adminDomain]);
            studentCount = await db.get("SELECT COUNT(*) as count FROM users WHERE role IN ('student','voter') AND email LIKE '%@' || ?", [adminDomain]);
            totalElections = await db.get("SELECT COUNT(DISTINCT e.id) as count FROM elections e JOIN election_access ea ON e.id = ea.election_id WHERE ea.email_domain = ?", [adminDomain]);
            nullifierAudit = await db.get("SELECT COUNT(*) as count FROM nullifier_audit");
        }
        res.json({
            stats: {
...
router.get("/users", authAdminMiddleware, async (req, res) => {
    try {
        let users;
        if (isSuperAdmin(req)) {
            users = await db.run("SELECT id, email, name, student_id, role, admin_domain, is_eligible, created_at FROM users ORDER BY created_at DESC");
        }
        else {
            const adminDomain = getAdminDomain(req);
            users = await db.run("SELECT id, email, name, student_id, role, admin_domain, is_eligible, created_at FROM users WHERE email LIKE '%@' || ? ORDER BY created_at DESC", [adminDomain]);
        }
...
router.get("/elections", authAdminMiddleware, async (req, res) => {
    try {
        let elections;
        if (isSuperAdmin(req)) {
            elections = await db.run("SELECT * FROM elections ORDER BY created_at DESC");
        }
        else {
            const adminDomain = getAdminDomain(req);
            elections = await db.run("SELECT DISTINCT e.* FROM elections e JOIN election_access ea ON e.id = ea.election_id WHERE ea.email_domain = ? ORDER BY e.created_at DESC", [adminDomain]);
        }
...
router.post("/elections", authAdminMiddleware, async (req, res) => {
    ...
    `, [election_id_blockchain, name, description, start_time, end_time]);
        ...
        if (adminDomain) {
            try {
                await db.exec("INSERT INTO election_access (election_id, email_domain) VALUES (?, ?)", [result.lastID, adminDomain]);
            }
            catch (e) {
 ...
 */
router.get("/registration-requests", authAdminMiddleware, async (req, res) => {
    try {
        ...
        const requests = await db.run(query, params);
 ...
 * @protected Admin only
 */
router.patch("/registration-requests/:id", authAdminMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        // Obtener solicitud
        const request = await db.get("SELECT * FROM registration_requests WHERE id = ?", [id]);
        if (!request) {
        ...
        if (action === 'approve') {
            ...
            // Insertar usuario
            const insertResult = await db.exec(`INSERT INTO users (email, password_hash, name, student_id, role, is_eligible, created_at)
         VALUES (?, ?, ?, ?, 'student', 1, CURRENT_TIMESTAMP)`, [request.email, passwordHash, request.full_name, request.student_id]);
            const newUserId = insertResult.lastID;
            // Actualizar solicitud como aprobada
            await db.exec("UPDATE registration_requests SET status = 'approved', reviewed_at = CURRENT_TIMESTAMP WHERE id = ?", [id]);
            // --- ASIGNACIÓN AUTOMÁTICA POR DOMINIO ---
            const domain = request.email.split('@')[1];
            if (domain) {
                const elections = await db.run('SELECT election_id FROM election_access WHERE email_domain = ? OR email_domain = "*"', [domain]);
                for (const election of elections) {
                    try {
                        await db.exec('INSERT INTO election_voters (election_id, user_id) VALUES (?, ?)', [election.election_id, newUserId]);
                    }
                    catch (e) {
            ...
            await db.exec("UPDATE registration_requests SET status = 'rejected', rejection_reason = ?, reviewed_at = CURRENT_TIMESTAMP WHERE id = ?", [reason, id]);
            res.json({
 ...
 * @protected Admin only
 */
router.post("/elections/:id/domains", authAdminMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        try {
            await db.exec("INSERT INTO election_access (election_id, email_domain) VALUES (?, ?)", [id, domain.trim()]);
 ...
 * @protected Admin only
 */
router.post("/elections/:id/voters", authAdminMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const user = await db.get("SELECT id FROM users WHERE email = ?", [email.trim()]);
        if (!user) {
            res.status(404).json({ error: "Usuario no encontrado" });
            return;
        }
        try {
            await db.exec("INSERT INTO election_voters (election_id, user_id) VALUES (?, ?)", [id, user.id]);
 ...
 * @protected Admin only
 */
router.post("/elections/:id/candidates", authAdminMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const result = await db.exec("INSERT INTO candidates (election_id, name, description) VALUES (?, ?, ?)", [id, name.trim(), description?.trim() || ""]);
