...
async function runMigration() {
    ...
    try {
        ...
        const info = await db.run("PRAGMA table_info(nullifier_audit)");
        const hasVoteChoice = info?.some((col) => col.name === "vote_choice");
        if (hasVoteChoice) {
            console.log("✅ Column vote_choice already exists. Skipping migration.");
            return;
        }
        // Add the column
        await db.exec(`ALTER TABLE nullifier_audit ADD COLUMN vote_choice TEXT DEFAULT NULL`);
...
//# sourceMappingURL=addVoteChoiceColumn.js.map